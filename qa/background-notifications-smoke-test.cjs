...
assert(app.includes("const APP_VERSION = '1.3.30-release-v91'"), 'App should expose the notification release version');
...
for (const source of [worker, publicWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v91-ops-stability'"), 'Service worker cache should be bumped for notification release');
...
assert(version.activeVersion === '1.3.30', 'Release version should be 1.3.30');
assert(version.activeVersionCode === 43, 'Release build should be 43');
assert(version.changelog.some(item => item.includes('Fejlcenter')), 'Changelog should mention the ops stability update');
