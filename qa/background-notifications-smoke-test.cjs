...
assert(app.includes("const APP_VERSION = '1.3.23-release-v84'"), 'App should expose the notification release version');
...
for (const source of [worker, publicWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v84-install-permissions'"), 'Service worker cache should be bumped for notification release');
...
assert(version.activeVersion === '1.3.23', 'Release version should be 1.3.23');
assert(version.activeVersionCode === 36, 'Release build should be 36');
assert(version.changelog.some(item => item.includes('Android spoerger')), 'Changelog should mention install-time notification permission');
