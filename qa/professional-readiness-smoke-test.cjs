...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v99-direct-chat-rest-fix'"), 'Service worker cache version should be bumped');
...
assert(app.includes(`const APP_VERSION = '${version.activeVersion}-release-v99'`), 'App version should be visible in code');
