...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v90-production-cleanup'"), 'Service worker cache version should be bumped');
...
assert(app.includes(`const APP_VERSION = '${version.activeVersion}-release-v90'`), 'App version should be visible in code');
