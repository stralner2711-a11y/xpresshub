...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v95-update-detection-fix'"), 'Service worker cache version should be bumped');
...
assert(app.includes(`const APP_VERSION = '${version.activeVersion}-release-v95'`), 'App version should be visible in code');
