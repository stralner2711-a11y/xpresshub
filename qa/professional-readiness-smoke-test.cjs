...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v59-control-cleanup'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.2.8-release-v59'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.2.8'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 11'), 'APK version code should be visible in code');
