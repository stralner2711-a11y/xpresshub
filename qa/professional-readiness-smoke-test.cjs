...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v68-standard-password'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.3.7-release-v68'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.3.7'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 20'), 'APK version code should be visible in code');
