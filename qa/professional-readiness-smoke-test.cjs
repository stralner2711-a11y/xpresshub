...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v65-info-rebuild'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.3.4-release-v65'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.3.4'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 17'), 'APK version code should be visible in code');
