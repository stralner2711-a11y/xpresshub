...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v101-update-detection'"), 'Service worker cache version should be bumped');
...
assert(app.includes(`const APP_VERSION = '${version.activeVersion}-release-v101'`), 'App version should be visible in code');
