...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v56-action-guards'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.2.5-release-v56'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.2.5'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 8'), 'APK version code should be visible in code');
