...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v62-info-simple'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.3.1-release-v62'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.3.1'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 14'), 'APK version code should be visible in code');
