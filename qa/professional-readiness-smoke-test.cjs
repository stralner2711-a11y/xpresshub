...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v64-security-update'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.3.3-release-v64'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.3.3'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 16'), 'APK version code should be visible in code');
