...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v60-update-modal'"), 'Service worker cache version should be bumped');
...
assert(app.includes("const APP_VERSION = '1.2.9-release-v60'"), 'App version should be visible in code');
assert(app.includes("const APP_DISPLAY_VERSION = '1.2.9'"), 'APK display version should be visible in code');
assert(app.includes('const APP_VERSION_CODE = 12'), 'APK version code should be visible in code');
