...
for (const source of [worker, rootWorker]) {
  assert(source.includes("CACHE_NAME = 'xpressintra-v100-github-release-robustness'"), 'Service worker cache version should be bumped');
  ...
  assert(source.includes("caches.match('./index.html')"), 'Service worker offline fallback should use index.html');
...
assert(app.includes(`const APP_VERSION = '${version.activeVersion}-release-v100'`), 'App version should be visible in code');
