...
assert(app.includes("const APP_DISPLAY_VERSION = '1.3.12'"), 'App should expose human APK version');
assert(app.includes('const APP_VERSION_CODE = 25'), 'App should compare numeric Android version codes');
...
assert(version.activeVersion === '1.3.12', 'version.json should expose activeVersion');
assert(version.activeVersionCode === 25, 'version.json should expose activeVersionCode');
...
assert(version.previousStableVersion === '1.3.11', 'version.json should keep the previous stable version for rollback');
assert(version.previousStableApkDownloadUrl.includes('/v1.3.11/'), 'version.json should expose previous stable APK for rollback');
