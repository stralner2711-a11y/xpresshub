...
assert(version.activeVersion === '1.3.25', 'version.json should expose activeVersion');
assert(version.activeVersionCode === 38, 'version.json should expose activeVersionCode');
...
assert(version.previousStableVersion === '1.3.24', 'version.json should keep the previous stable version for rollback');
assert(version.previousStableApkDownloadUrl.includes('/v1.3.24/'), 'version.json should expose previous stable APK for rollback');
assert(version.changelog.some(item => item.includes('Forsiden skifter nu automatisk')), 'version.json should explain the day greeting update');
