...
assert(version.activeVersion === '1.3.23', 'version.json should expose activeVersion');
assert(version.activeVersionCode === 36, 'version.json should expose activeVersionCode');
...
assert(version.previousStableVersion === '1.3.22', 'version.json should keep the previous stable version for rollback');
assert(version.previousStableApkDownloadUrl.includes('/v1.3.22/'), 'version.json should expose previous stable APK for rollback');
assert(version.changelog.some(item => item.includes('Android spoerger')), 'version.json should explain the install permission update');
