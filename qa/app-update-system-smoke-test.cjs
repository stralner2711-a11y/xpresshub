...
assert(version.activeVersion === '1.3.30', 'version.json should expose activeVersion');
assert(version.activeVersionCode === 43, 'version.json should expose activeVersionCode');
...
assert(version.previousStableVersion === '1.3.29', 'version.json should keep the previous stable version for rollback');
assert(version.previousStableApkDownloadUrl.includes('/v1.3.29/'), 'version.json should expose previous stable APK for rollback');
assert(version.changelog.some(item => item.includes('Creator driftspanel')), 'version.json should explain the practical drift update');
