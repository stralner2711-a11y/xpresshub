...
assert(fs.existsSync('vite.active.config.js'), 'vite.active.config.js should exist for bundled dependencies');
...
assert(gitignore.includes('android-active/offline-maven/'), 'active local Gradle cache should be ignored by Git');
assert(releaseScript.includes("android-active\\offline-maven"), 'release script should exclude active local Gradle cache from GitHub package');
assert(releaseScript.includes("android-active\\app\\build"), 'release script should exclude active Android build output from GitHub package');
assert(releaseScript.includes("ios-active\\App\\build"), 'release script should exclude active iOS build output from GitHub package');
