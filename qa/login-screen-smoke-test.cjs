...
assert(loginBlock.includes('const canUseInviteSignup = Boolean(backend.ready && inviteContext.valid)'), 'Invite-specific signup requires a valid link');
assert(loginBlock.includes('data-action="signup-access-request"'), 'Public signup should allow approval requests');
assert(app.includes('name="fullName"'), 'Signup should ask for the employee name');
assert(loginBlock.includes('data-action="signup-invite-profile"'), 'Login should support invite-link employee signup');
assert(loginBlock.includes('formnovalidate'), 'Invite signup should bypass empty password validation on the login form');
...
assert(app.includes('openStandardSignupPasswordModal'), 'Invite signup should open a personal password step before Supabase signup');
assert(!app.includes('TEMPORARY_EMPLOYEE_PASSWORD'), 'App must not keep a shared onboarding password in source');
...
assert(app.includes('Opret konto kræver et invitationslink fra chef eller creator'), 'Signup without invite link should be blocked');
