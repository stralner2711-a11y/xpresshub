...
create table if not exists public.retention_policies (
  ...
  area text not null default 'Andet',
  description text not null,
...
alter table public.retention_policies
  add column if not exists area text not null default 'Andet';
...
create index if not exists app_telemetry_daily_date_idx on public.app_telemetry_daily (metric_date desc);
...
begin
  ...
  delete from public.app_telemetry_daily where metric_date < current_date - 90;
...
on public.app_telemetry_daily for select to authenticated using ((select private.is_admin()));
...
values
  ('live_gps', 'Live GPS', 1, 'GPS', 'Seneste position overskrives løbende; historik bør undgås i MVP', true),
  ('pickup_location', 'Afhentningsdeling', 1, 'GPS', 'Midlertidig GPS-deling afsluttes ved udløb eller færdigmelding', true),
  ('chat_messages', 'Chatbeskeder', 730, 'Chat', 'Intern chat bør have en aftalt slette- eller arkivfrist', false),
  ('media', 'Billeder', 365, 'Billeder', 'Billeder gemmes efter dokumentationsformål og slettes når formålet ophører', false),
  ('audit_log', 'Audit-log', 730, 'Sikkerhed', 'Administrative ændringer gemmes af hensyn til sikkerhed og ansvarlighed', false),
  ('app_telemetry', 'Anonym teknisk driftsstatistik', 90, 'Drift', 'Sammenlagte tekniske tællere uden bruger-id, mail, beskedtekst, GPS, billeder eller rå fejltekst', true)
on conflict (key) do update
set label = excluded.label, retention_days = excluded.retention_days, area = excluded.area, description = excluded.description, auto_delete = excluded.auto_delete;
